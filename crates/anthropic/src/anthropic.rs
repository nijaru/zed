...
pub async fn stream_completion_oauth(
    client: &dyn HttpClient,
    api_url: &str,
    access_token: &str,
    request: Request,
) -> Result<BoxStream<'static, Result<Event, AnthropicError>>, AnthropicError> {
    stream_completion_oauth_with_rate_limit_info(client, api_url, access_token, request)
        .await
        .map(|output| output.0)
}
...
pub async fn stream_completion_oauth_with_rate_limit_info(
    client: &dyn HttpClient,
    api_url: &str,
    access_token: &str,
    request: Request,
...
> {
    ...
    let beta_headers = Model::from_id(&request.base.model)
        .map(|model| {
            let headers = model.beta_headers();
            // Add OAuth beta header for Claude Pro/Max support
            if headers.contains("oauth-2025-04-20") {
                headers
            } else {
                format!("{},oauth-2025-04-20", headers)
            }
        })
        .unwrap_or_else(|_| {
            format!("{},oauth-2025-04-20", Model::DEFAULT_BETA_HEADERS.join(","))
        });
    ...
    if response.status().is_success() {
        ...
        match serde_json::from_str::<Event>(&body) {
            Ok(Event::Error { error }) => Err(AnthropicError::ApiError(error)),
            Ok(_) => Err(AnthropicError::HttpResponseError {
                status_code: response.status(),
                message: format!("Unexpected response format: {}", body),
            }),
